...
public class Handler implements Runnable {
    ...
    public Handler(AtomicReference<Session> session,AtomicReference<JsonObject> message, AtomicReference<JsonObject> googleAccessToken,AtomicReference<Database> database, AtomicBoolean hasEmails){
    ...
    private void addFavorite(JsonObject message){
        ...
        String attachment = filter.get("Attachment").getAsString();
        boolean seen = filter.get("Seen").getAsBoolean();

        try {
            Date date = new SimpleDateFormat("MM/dd/yyyy").parse(stringDate);
            database.get().insertUserFavourites(favoriteName,folderName,date,interval,attachment,seen);
    ...
    private void handleGraphs(JsonObject message) throws InterruptedException {
        ...
        List<Email> emails = database.get().getEmailByFilter(folderName,date,endDate,attachment,seen);
        List<UserFolder> folders = database.get().importFolders();
