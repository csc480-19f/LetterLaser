...
public class TimeBetweenRepliesCallable implements Callable {
	...
	/**
	 * This method assumes emails do not repeat in the emails array list. This
	 * method returns a 2-d array where: indexes [0][x] are times between sent.
	 * indexes [1][x] are times between received. indexes [x][0] are for Sunday.
	 * indexes [x][1] are for Monday. ... indexes [x][6] are for Saturday.
	 */
	@Override
	public Object call() throws Exception {
		// The returned value, a 2-d array that stores the average time between sent at
		// [0][x], and received at [1][x].
		double[][] times = new double[2][7];
		// This is the total number of emails sent, corresponding to the times in the
		// array above.
		...
		/*
		 * This section converts the emails array list into a hash map. This hash map
		 * has an array list stored as its value at the subject key as emails in a
		 * conversation all have the same subject, at least for gmail.
		 */
		HashMap<String, ArrayList<Email>> subjects = new HashMap<>();
		HashMap<String, Integer> addresses = new HashMap<>();
		for (Email e : emails) {
			// The OR bit here takes any email with subject "Re: " and treats is as its
			// subject w/o "Re: "
		...
		for (String key : keys) {
			...
			boolean sent = false;
			if (conversation.get(0).getFrom().get(0).getEmailAddress().equalsIgnoreCase(user))
				sent = true;

			// And here we figure out how long it took between emails and store it
			// appropriately. It gets averaged later.
			if (sent) {
				// Since the user sent the first email, time between replies for sent starts at
				// index 0, GOTO 111
				for (int q = 0; q < conversation.size(); q += 2) {
					int dayIndex = findDay(conversation.get(q));
					totals[0][dayIndex]++;

					// If there is an email that has been received as a result of sending the email
					// at index q
			...
			// Similarly, the same process as above is applied here, except that sent begins
			// at 1 and received at 0.
			else {
				for (int q = 1; q < conversation.size(); q += 2) {
					int dayIndex = findDay(conversation.get(q));
					totals[0][dayIndex]++;
		...
		// Here we just divide the total at each index and divide by its total count in
		// totals to calculate the average.
		for (int q = 0; q < 7; q++) {
	...
	/**
	 * This method finds the hour an email is received. Minutes are represented as
	 * decimal values after the whole.
	 * 
	 * @param start
	 *            The email prior to the one received / The email sent.
	 * @param end
	 *            The email received / The email after the one sent.
	 * @return The hour received as a number of hours, whose decimal is equivalent
	 *         to the minutes.
	 */
