...
/**
 * @author PhoenixBoisnier This runnable calculates the folder populations. It
 *         assumes that the list of emails provided all exist within a folder
 *         within a folder, and assumes that all the emails in the relevant
...
public class FolderCallable implements Callable {
	...
	/**
	 * This method creates a folderMapper class, which calculates the data for us in
	 * the constructor.
	 */
	@Override
	...
	/**
	 * This is the folderMapper class. It happened to be the way that made the most
	 * sense to myself to organize the data. This class has protected results as:
	 * outerCount; mapping the relevant folder names to their email counts,
	 * folderToParent; mapping the relevant folders to their parent folder, and
	 * keyRing; the complete list of keys used for outerCount and folderToParent.
	 */
			...
			for (Email e : emails) {
				...
				if (parent != null) {
					...
					try {
						...
						// Like above, an email that does not gave a grandparent does not meet our
						// assumptions.
					...
					if (grandParent != null) {
						...
						// Otherwise, we make a new entry in our map.
							...
							// And since we only care about emails that get this far, we only store the
							// folder info now.
							keyRing.add(e.getFolder().getFolder().getName());
						}
						// In either case, since the parent exists, we'll add it to the relationship
						// map.
