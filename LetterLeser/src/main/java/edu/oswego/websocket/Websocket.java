...
//		}
	...
	private void login(Session session, String email, String pass, StorageObject storageObject) {
		...
		if(!connection){
			messenger.sendErrorMessage(session,"sorry but we can't connect to your email. maybe invalid email n password");
			return;
		}
		...
		messenger.sendUpdateStatusMessage(session,"we found your inbox and getting your folders: this may take a second");
	...
	private void calculations(Session session,String email, List<Email> emailList){
		...
		try {
			...
			messenger.sendUpdateStatusMessage(session,"got the number of emails\ngetting the amount you send and recieve");
			sendNRec = (JsonObject) snrc.get(2, TimeUnit.MINUTES);
			messenger.sendUpdateStatusMessage(session,"got the amount of emails you send and receive \ngetting your the time between your emails");
		...
		messenger.sendUpdateStatusMessage(session,"we got all your emails, we are compiling the data and will be ready to send momentarily");
