...
/**
 * All test for filter_settings method
 * 
 * @author Jimmy Nguyen
 * @since 11/10/2019
 */
class FilterSettingsTest {
	...
	void testFilterInsert() {
		...
		db.query(
				"INSERT INTO user_favourites (fav_name, user_id, filter_settings_id) VALUE ('Pepperoni Nips', "
						+ db.getUser().getId() + ", 1);");
	...
	void testGetEmailByFilter() {
		...
		db.query("INSERT INTO folder (fold_name) VALUE ('poopsac')");
		db.query("INSERT INTO email (has_attachment, date_received, seen, folder_id) VALUE (1, " + dateBetween
				+ ", 1, 1)");
		db.query("INSERT INTO email (has_attachment, date_received, seen, folder_id) VALUE (1, " + dateBetween
				+ ", 1, 1)");
		db.query("INSERT INTO email (has_attachment, date_received, seen, folder_id) VALUE (1, " + dateBetween
	...
	@Test
	void testGetEmailByFilterFail() {
		String dateUno = "2020-10-10 00:00:00";
		String dateBetween = "'2010-11-09 00:00:00'";
		String dateDos = "2020-11-11 00:00:00";
		db.query("INSERT INTO folder (fold_name) VALUE ('poopsac')");
		db.query("INSERT INTO email (has_attachment, date_received, seen, folder_id) VALUE (1, " + dateBetween
				+ ", 1, 1)");
		db.query("INSERT INTO email (has_attachment, date_received, seen, folder_id) VALUE (1, " + dateBetween
		...
		assertEquals(db.getEmailByFilter(true, dateUno, dateDos, true, "poopsac").size(), 0);
	}
