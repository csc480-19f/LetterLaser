...
@ServerEndpoint("/engine/{validation}")
public class Websocket {
    ...
    @OnOpen
    public void onOpen(Session session, @PathParam("validation") String validation) {
        if(!validation.equals("accept")){//They need to make
            try {
                session.close();
            } catch (IOException e) {
                e.printStackTrace();
